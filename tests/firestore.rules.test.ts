...
describe('notices(삭제된 공지 기능)', () => {
  it('운영진도 회원도 공지를 읽거나 쓸 수 없다(기본 거부)', async () => {
    const admin = testEnv.authenticatedContext('admin-x', googleUser('admin-x')).firestore();
    const member = testEnv.authenticatedContext('member-a', googleUser('member-a')).firestore();
    await assertFails(setDoc(doc(admin, 'notices', 'n1'), { title: '공지', body: '본문', authorId: 'admin-x', authorName: '운영진', createdAt: serverTimestamp(), updatedAt: serverTimestamp() }));
    await assertFails(getDoc(doc(member, 'notices', 'n1')));
  });
});
