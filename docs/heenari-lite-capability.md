...
현재 활성 구현 게이트: `docs/gates/HEENARI-FB-02-RESERVATION.md`
...
- 전체 예약과 동아리 일정을 조회한다.
- 비어 있는 30분 슬롯을 하나 이상 선택해 예약한다.
- 자신이 만든 미래 예약을 수정하거나 취소한다.
- 지난 예약은 조회만 한다.
...
- 모든 예약을 수정하거나 취소한다.
- 동아리 일정을 생성, 수정, 삭제한다.
- 관리자 권한 부여 방식은 별도 보안 게이트에서 결정한다.
...
홈에서는 다음 예약과 다음 동아리 일정을 각각 한 건씩 보여준다. 일정 화면은
선택한 기간의 예약과 동아리 일정을 시간순으로 합쳐 보여주며, 사용자는
`전체 / 예약 / 동아리 일정`으로 필터링할 수 있다.
...
하단 내비게이션은 `홈 / 예약 / 일정 / 내 정보` 네 항목으로 고정한다.
...
- 예약 상태: 색상만이 아니라 `예약 가능`, `내 예약`, `예약됨` 텍스트 병기
...
- 하단 내비게이션: 엄지손가락으로 접근 가능한 네 항목 고정
...
초기 버전은 모든 검증된 Google 계정을 일반 회원으로 취급한다. 관리자 권한은
클라이언트 입력에 맡기지 않고 후속 게이트에서 Custom Claims 등 신뢰 가능한
권한 소스를 도입할 때 활성화한다.
...
- 관리자 쓰기 권한은 신뢰 가능한 권한 소스가 정해질 때까지 제공하지 않는다.
- 기본 규칙은 모든 접근 거부다.
