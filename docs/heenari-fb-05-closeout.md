# HEENARI-FB-05 홈 다음 합주 — Closeout
...
## 공지 기능 삭제 (2026-09-24, 사용자 요청)

- 위 결과 중 **동아리 공지**(홈 공지 칸, `/notices`, 공지 시트, `notices` Rules·테스트)를 모두 지웠다.
  빨간 카드(다음 합주)와 textarea 글꼴 수정은 남겼다.
- 삭제 전 운영 DB의 `notices` 문서 수를 확인했다: 0건(정리할 데이터 없음).
- `notices`는 Rules 기본 거부로 돌아갔고, 운영진·회원 모두 읽기·쓰기가 거부되는 것을 에뮬레이터
  테스트 1개로 고정했다.
- 배포 순서: 앱(공지 화면 제거)을 먼저 머지·배포한 뒤 Rules를 배포했다. 반대로 하면 예전 앱이
  잠시 "공지를 불러오지 못했어요"를 보이기 때문이다.
